...
################################################################
class CEODLoss:
    def __init__(self, bs, keep_dim=1, gamma=0.4, lamda=1, block_size=512, sample_size=1024):
        """
        Initialize the CEOD loss function.
        Args:
            keep_dim: Number of feature dimensions to keep (default: 1).
            gamma: Bandwidth parameter for the Gaussian kernel.
            lamda: Regularization coefficient.
            block_size: Block size for chunked computation.
            sample_size: Number of samples for random sampling.
        """
        self.bs = bs
    ...
    def _reshape(self, x):
        """
        Flatten high-dimensional input into (total_samples, features) format.
        Args:
            x: Input tensor with shape [..., features].
        Returns:
            Flattened tensor with shape [total_samples, features].
        """
        if x.dim() > 2:
    ...
    def kernel(self, X, X2):
        """
        Compute the Gaussian kernel matrix (supports chunked computation).
        Args:
            X: Source domain input with shape [..., n_features].
            X2: Target domain input with shape [..., n_features].
        Returns:
            K: Kernel matrix with shape [N, M], where N and M are the number of samples.
        """
        # Reshape inputs to 2D tensors and cast to float32
        # X = self._reshape(X).float()  # [N, n_features], cast to float32
        # X2 = self._reshape(X2).float()  # [M, n_features], cast to float32

        # Transpose feature matrices
        X = X.t()  # [n_features, N]
        X2 = X2.t()  # [n_features, M]

        # Compute squared norms
        X_sq = torch.sum(X ** 2, dim=0)  # [N,]
        X2_sq = torch.sum(X2 ** 2, dim=0)  # [M,]

        # Compute distance matrix in chunks
        # print(X.shape[1], X2.shape[1])
        # K = torch.zeros(X.shape[1], X2.shape[1], device=X.device, dtype=torch.float32)
        # for i in range(0, X.shape[1], self.block_size):
        #     for j in range(0, X2.shape[1], self.block_size):
        #         X_block = X[:, i:i+self.block_size]
        #         X2_block = X2[:, j:j+self.block_size]
        #         D_block = X_sq[i:i+self.block_size].unsqueeze(1) + \
        #                   X2_sq[j:j+self.block_size].unsqueeze(0) - \
        #                   2 * torch.mm(X_block.t(), X2_block)
        #         K[i:i+self.block_size, j:j+self.block_size] = torch.exp(-self.gamma * D_block)
        D = X_sq.unsqueeze(1) + X2_sq.unsqueeze(0) - 2 * torch.mm(X.t(), X2)
        K = torch.exp(-self.gamma * D)
        return K

    def CEOD(self, X_p_list, Y_p, X_q_list, Y_q):
        """
        Compute the CEOD loss (supports large-scale data).
        Args:
            X_p_list: List of source domain features.
            Y_p: Source domain labels.
            X_q_list: List of target domain features.
            Y_q: Target domain labels.
        Returns:
            loss: Computed CEOD loss value.
        """
        total_loss = 0
        layer_num = 1  # Original code uses only the first layer

        for i in range(layer_num):
            # Flatten features and cast to float32
            X_p = self._reshape(X_p_list[i]).float()  # [N, n_features]
            X_q = self._reshape(X_q_list[i]).float()  # [M, n_features]
            Y_p_ = self._reshape(Y_p).float()  # [N, label_features]
            Y_q_ = self._reshape(Y_q).float()  # [M, label_features]

            # Random sampling to reduce computation
            # if X_p.shape[0] > self.sample_size:
            #     idx_p = torch.randperm(X_p.shape[0])[:self.sample_size]
            #     X_p = X_p[idx_p]
            #     Y_p_ = Y_p_[idx_p]
            #
            # if X_q.shape[0] > self.sample_size:
            #     idx_q = torch.randperm(X_q.shape[0])[:self.sample_size]
            #     X_q = X_q[idx_q]
            #     Y_q_ = Y_q_[idx_q]

            # Compute kernel matrices
            K_xp_xp = self.kernel(X_p, X_p)
            K_xq_xq = self.kernel(X_q, X_q)
            K_xq_xp = self.kernel(X_q, X_p)
            K_yp_yq = self.kernel(Y_p_, Y_q_)
            K_yq_yq = self.kernel(Y_q_, Y_q_)
            K_yp_yp = self.kernel(Y_p_, Y_p_)

            # Add regularization term (avoid explicit construction of identity matrix)
            K_xp_xp_reg = K_xp_xp + self.bs * self.lamda * torch.eye(K_xp_xp.size(0), device=K_xp_xp.device, dtype=torch.float32)
            K_xq_xq_reg = K_xq_xq + self.bs * self.lamda * torch.eye(K_xq_xq.size(0), device=K_xq_xq.device, dtype=torch.float32)

            # Compute inverse of regularized kernel matrices
            inv_K_xp_reg = torch.inverse(K_xp_xp_reg)
            inv_K_xq_reg = torch.inverse(K_xq_xq_reg)

            # Compute the three loss components
            term1 = torch.trace(inv_K_xp_reg @ K_yp_yp @ inv_K_xp_reg @ K_xp_xp)
            term2 = torch.trace(inv_K_xq_reg @ K_yq_yq @ inv_K_xq_reg @ K_xq_xq)
            term3 = torch.trace(inv_K_xp_reg @ K_yp_yq @ inv_K_xq_reg @ K_xq_xp)

            total_loss += (term1 + term2 - 2 * term3)
